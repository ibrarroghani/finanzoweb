    ...
    <div className='relative flex flex-col'>
      <div className='relative py-2'>
        ...
        {icon ? (
          ...
          <p className='absolute left-4 top-[20%] text-10 text-gray-500'>
            {label}
          </p>
        ) : (
        ...
        <Controller
          ...
          render={({ field: { onChange, value } }) => (
            <input
              ...
              value={value}
              onChange={onChange}
              className={`${icon ? 'py-2 pl-8 pr-4' : labelPosition === 'outside' ? 'p-2.5' : 'px-4 py-6 pb-2'} no-spinner block w-full max-w-full rounded-md border border-[#eeeeee] bg-primary-light text-15 text-gray-900 placeholder-gray-400 focus:outline-1 focus:outline-green-300`}
            />
          )}
        />
